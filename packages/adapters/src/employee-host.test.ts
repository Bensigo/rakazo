...
import {
  type EmployeeHostOperation,
  EmployeeHostRegistry,
  employeeHostWorkspaceCwd,
  LocalEmployeeHostCompanion,
  LocalEmployeeHostReceiptSpool,
  PrismaEmployeeHostTransport,
  runEmployeeHostCompanion,
} from "./employee-host.js";
...
describe("employee host protocol", () => {
  it("preserves the server computer identity and scopes it to its owner", async () => {
    ...
    const transport = new PrismaEmployeeHostTransport({
      employeeHost: { findFirst },
    ...
    await expect(
      transport.provision(
        { botId: "employee-home", homePath: "/server-home", providerKind: "employee-host" },
        context,
      ),
  ...
  it("authenticates outbound poll and expires missed heartbeats", () => {
    ...
    expect(() => registry.poll("host-1", enrollment.enrollmentToken, 1101)).toThrow(
      /authentication/,
    );
  });
  ...
  it("rejects a wrong-space operation and stale fenced replay", () => {
    ...
    const enrollment = host(registry);
    const lease = registry.acquireLease(
      {
        hostId: "host-1",
        spaceId: "space-1",
        botId: "bot-1",
    ...
    const operation = registry.enqueue(
      {
        hostId: "host-1",
        spaceId: "space-1",
        botId: "bot-1",
    ...
    expect(registry.poll("host-1", enrollment.enrollmentToken, 1004)?.operationId).toBe(
  ...
  it("keeps operation identifiers inside the spool and preserves malformed evidence", async () => {
    ...
    await expect(spool.pending()).rejects.toThrow(
      /preserve it for inspection and repair or remove it/,
    );
  ...
  it("reconciles a lost terminal receipt response after restart without rerunning", async () => {
    ...
    const operation: EmployeeHostOperation = {
      ...
      request: {
        argv: [
          process.execPath,
          "-e",
          "require('node:fs').appendFileSync('marker.txt', 'once\\n')",
        ],
        cwd: ".",
      },
    };
    ...
    expect(await spool.pending()).toEqual([
      expect.objectContaining({ operationId: operation.operationId, status: "completed" }),
    ]);
