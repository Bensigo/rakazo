  ...
  discoverTools: async () => [
    {
      ...
      route: { connectorId: "composio", toolName: "issues.list" },
    },
  ],
...
test("managed provider MCP boundary authenticates and preserves calls without provider egress", async () => {
  ...
  const service = createProviderMcpHttpServer({
    token,
    port: 0,
    providers: { composio: provider, pipedream: undefined },
  });
  ...
  try {
    ...
    assert.deepEqual(await health.json(), {
      ...
      providers: { composio: true, pipedream: false },
    });
    ...
    const client = new ManagedProviderMcpClient({
      providerId: "composio",
    ...
    assert.deepEqual(events, [
      { type: "result", data: { tool: "issues.list", args: { limit: 1 } } },
    ]);
  } finally {
    await service.close();
  }
...
test("rejects malformed provider output and normalizes void lifecycle results", async () => {
  const token = "t".repeat(32);
  const malformed: ManagedConnectorProvider = {
    ...provider,
    catalog: async () => [{ bad: true }] as never,
  };
  const service = createProviderMcpHttpServer({
    token,
    port: 0,
    providers: { composio: malformed, pipedream: undefined },
  });
  await service.listen();
  const port = (service.http.address() as AddressInfo).port;
  try {
  ...
  assert.throws(
    () =>
      new ManagedProviderMcpClient({
        providerId: "composio",
        endpoint: "http://provider-mcp:3180/mcp",
        token,
      }),
    /HTTPS unless loopback/,
  );
  assert.doesNotThrow(
    () =>
      new ManagedProviderMcpClient({
...
test("disabled providers have empty read capabilities but fail closed for mutations", async () => {
  ...
  const service = createProviderMcpHttpServer({
    token,
    port: 0,
    providers: { composio: undefined, pipedream: undefined },
  });
  await service.listen();
  const port = (service.http.address() as AddressInfo).port;
  try {
    ...
    await assert.rejects(
      () =>
        client.begin({ provider: "github", redirectUrl: "https://example.test/callback" }, context),
      /operation failed|request failed|not configured/i,
    );
    await assert.rejects(async () => {
      for await (const _event of client.execute(
        { tool: "x", args: {}, executionId: "x" },
        context,
      )) {
      }
    }, /operation failed|request failed|not configured/i);
    await assert.rejects(
      () => client.revoke("missing", context),
      /operation failed|request failed|not configured/i,
    );
  } finally {
    await service.close();
...
test("MCP preserves raw webhook ACK/events and isolates concurrent collectors", async () => {
  ...
  const fake = (): MessagingSurface => {
    ...
    const surface: MessagingSurface = {
      describe: () => ({
        ...
        contractVersion: "1",
        adapterVersion: "test",
        capabilities: { providers: ["fake"] },
      }),
      platforms: () => [
        { provider: "fake", capabilities: { direct: true, groups: false, typing: false } },
      ],
      onInbound: (next) => {
        sink = next;
        sinks.push(next);
      },
      handleWebhook: async (_provider, request) => {
        ...
        return new Response("ACK:" + body, { status: 202, headers: { "x-provider": "fake" } });
      },
      sendToThread: async () => ({ handle: "sent" }),
      openDirectThread: async () => "fake:dm",
  ...
  const service = createProviderMcpHttpServer({
    ...
    port: 0,
    providers: { composio: undefined, pipedream: undefined },
    services: { messagingFactory: fake },
  });
  await service.listen();
  const port = (service.http.address() as AddressInfo).port;
  try {
    const rpc = new ManagedProviderMcpClient({
      providerId: "composio",
      endpoint: `http://127.0.0.1:${port}/mcp`,
      token,
    });
    ...
    left.onInbound(async (event) => {
      leftEvents.push(event);
    });
    right.onInbound(async (event) => {
      rightEvents.push(event);
    });
    const [leftResponse, rightResponse] = await Promise.all([
      left.handleWebhook(
        "fake",
        new Request("https://fake.test/hook", { method: "POST", body: "left" }),
      )!,
      right.handleWebhook(
        "fake",
        new Request("https://fake.test/hook", { method: "POST", body: "right" }),
      )!,
    ]);
    assert.equal(await leftResponse.text(), "ACK:left");
    assert.equal(await rightResponse.text(), "ACK:right");
    assert.equal(leftEvents.length, 1);
    assert.equal(rightEvents.length, 1);
    assert.equal((leftEvents[0] as { handle: string }).handle, "left");
    assert.equal((rightEvents[0] as { handle: string }).handle, "right");
...
test("MCP routes email and scoped push token without persisting it", async () => {
  ...
  const service = createProviderMcpHttpServer({
    ...
    services: {
      ...
      push: async (destination, message) => {
  ...
  try {
    ...
    const email = new ManagedEmailMcpClient(rpc);
    await email.send({ to: "person@example.com", subject: "Hello", text: "Body" });
    await email.drain();
    const notification = new ManagedNotificationMcpClient(rpc, async (userId) =>
      userId === "user-1" ? "ExponentPushToken[secret]" : undefined,
    );
    await notification.send(
      { kind: "completion", title: "Done", body: "Finished", botId: "bot-1", threadId: "thread-1" },
      { ...context, userId: "user-1" },
    );
    assert.equal(emailCount, 1);
    assert.equal(pushToken, "ExponentPushToken[secret]");
  } finally {
    await service.close();
  }
});
...
test("delivery failures never expose provider secrets or inbound event data", async () => {
  ...
  const failingMessaging = (): MessagingSurface => {
    ...
    return {
      ...
      platforms: () => [
        { provider: "fake", capabilities: { direct: true, groups: false, typing: true } },
      ],
      onInbound: (next) => {
        sink = next;
      },
      ...
      sendToThread: async () => {
        throw new Error(`vendor request ${secret}`);
      },
  ...
  const service = createProviderMcpHttpServer({
    ...
    services: {
      ...
      email: {
        ...
        send: async () => {
          throw new Error(`smtp password ${secret}`);
        },
        drain: async () => {
          throw new Error(`smtp password ${secret}`);
        },
      ...
      push: async () => {
        throw new Error(`expo bearer secret ${secret}`);
      },
    },
  });
  await service.listen();
  ...
  try {
    ...
    messaging.onInbound(async () => {
      throw new Error(`sink event ${secret}`);
    });
    const operations = [
      ...
      () =>
        (async () => {
          await messaging.handleWebhook(
            "fake",
            new Request("https://fake.test/hook", { method: "POST", body: "event" }),
          );
        })(),
      () =>
        new ManagedEmailMcpClient(rpc).send({
      ...
      () =>
        new ManagedNotificationMcpClient(rpc, async () => "ExponentPushToken[private]").send(
          { kind: "completion", title: "Done", body: "Body", botId: "bot-1", threadId: "thread-1" },
          context,
        ),
    ];
    for (const operation of operations) {
      await assert.rejects(operation(), (error: unknown) => {
        assert.match(
          String(error),
          /Managed (delivery operation failed|provider operation failed|provider failed)/i,
        );
        assert.doesNotMatch(
          String(error),
          /secret\.example|signing-material|password|bearer secret|private/i,
        );
        return true;
      });
    }
