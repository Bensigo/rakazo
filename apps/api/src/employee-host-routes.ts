...
const receiptSchema = z
  .object({
    ...
    lease: z.object({
      computerId: z.string().min(1).max(256),
    ...
    result: z.object({
      ...
      code: z.number().int().min(-1_000_000).max(1_000_000),
    }),
  })
  .strict()
  .superRefine((receipt, context) => {
    const expected =
      receipt.result.code === 0 ? "completed" : receipt.result.code === 125 ? "unknown" : "failed";
    if (receipt.status !== expected)
      context.addIssue({
        code: "custom",
        path: ["status"],
        message: "Receipt status does not match its process result",
      });
    if (receipt.completedAt < receipt.acceptedAt)
      context.addIssue({
        code: "custom",
        path: ["completedAt"],
        message: "Receipt completedAt precedes acceptedAt",
      });
  });
...
export async function enrollEmployeeHost(
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await tx.computer.create({
        data: {
          ...
          userId: actor.userId,
          scope: "dedicated",
          scopeKey: `employee-host:${input.hostId}`,
          homeKey: `employee-host:${input.hostId}`,
          kind: "employee-host",
          providerRef: input.hostId,
          state: "running",
        },
      });
      await tx.employeeHost.create({
        data: {
          hostId: input.hostId,
          spaceId: actor.spaceId,
    ...
    if (error && typeof error === "object" && "code" in error && error.code === "P2002")
      throw new EmployeeHostEnrollmentConflictError("Host already enrolled");
    throw error;
  }
...
export function mountEmployeeHostRoutes(
  ...
  app.post("/employee-hosts/enroll", async (c) => {
    ...
    if (!input?.hostId || !input.name || !input.platform || !input.workspaceRoot)
      return c.json({ error: "Invalid enrollment" }, 400);
    if (
      [input.hostId, input.name, input.platform, input.workspaceRoot].some(
        (value) => value.length > 512,
      )
    )
      return c.json({ error: "Invalid enrollment" }, 400);
    if (!/^[a-z0-9][a-z0-9._:-]{0,127}$/iu.test(input.hostId))
      return c.json({ error: "Invalid enrollment" }, 400);
    const { hostId, name, platform, workspaceRoot } = input;
    try {
      return c.json(
        await enrollEmployeeHost(deps.prisma, actor, {
          hostId,
          name,
          platform,
          workspaceRoot,
  ...
  async function host(c: Context, allowExpired = false) {
    ...
    if (
      ...
      (!allowExpired && record.expiresAt <= new Date())
    )
      return null;
    return { record, token };
  }
  ...
  app.post("/employee-hosts/:hostId/heartbeat", async (c) => {
    ...
    const capabilities = (
      input?.capabilities && typeof input.capabilities === "object"
        ? input.capabilities
        : authenticated.record.capabilities
    ) as Prisma.InputJsonValue;
    await deps.prisma.employeeHost.update({
      where: { hostId: c.req.param("hostId") },
      data: { capabilities, lastSeenAt: now, expiresAt: new Date(now.getTime() + TTL_MS) },
    });
  ...
  app.post("/employee-hosts/:hostId/poll", async (c) => {
    ...
    if (!authenticated) return c.json({ error: "Unauthorized" }, 401);
    const candidate = await deps.prisma.employeeHostOperation.findFirst({
      where: {
        hostId: c.req.param("hostId"),
        computerId: authenticated.record.computerId,
        status: "accepted",
        spaceId: authenticated.record.spaceId,
      },
    ...
    const operation =
      candidate &&
      (
        await deps.prisma.employeeHostOperation.updateMany({
          where: { id: candidate.id, status: "accepted" },
    ...
    return c.json({
      operation: {
        ...
        lease: {
          ...
          expiresAt: operation.acceptedAt.getTime() + TTL_MS,
        },
        kind: "exec",
  ...
  app.post("/employee-hosts/:hostId/receipts/:operationId", async (c) => {
    const authenticated = await host(c);
    if (!authenticated) return c.json({ error: "Unauthorized" }, 401);
    const operation = await deps.prisma.employeeHostOperation.findFirst({
      where: {
        hostId: c.req.param("hostId"),
    ...
    if (operation.status !== "accepted" && operation.status !== "dispatched")
      return c.json({ ok: true, status: operation.status });
    ...
    if (
      ...
      receipt.hostId !== operation.hostId ||
      receipt.lease.runId !== operation.runId ||
      receipt.lease.fence !== operation.fence ||
      receipt.lease.computerId !== operation.computerId
    )
      return c.json({ error: "Stale receipt" }, 409);
    ...
    const lease = await deps.prisma.computerExecutionLease.findFirst({
      where: {
        ...
        computer: { spaceId: operation.spaceId },
      },
    });
    if (!lease) return c.json({ error: "Stale receipt" }, 409);
    const updated = await deps.prisma.employeeHostOperation.updateMany({
      where: {
        id: operation.id,
        hostId: operation.hostId,
        runId: operation.runId,
