...
integration("employee host routes (PostgreSQL)", () => {
  ...
  it("enrolls, claims once, fences receipts, and persists completion", async () => {
    ...
    await prisma.computer.create({ data: { id: ids.computer, spaceId: ids.space, userId: ids.user, scope: "dedicated", scopeKey: `host-test-${ids.computer}`, homeKey: `host-home-${ids.computer}`, kind: "employee-host" } });
    ...
    await prisma.employeeHostOperation.create({ data: { operationId, hostId, computerId: ids.computer, spaceId: ids.space, botId: ids.bot, runId: ids.run, fence: 7, request: { argv: ["printf", "ok"] } } });
    ...
    expect((await app.request(`http://test/employee-hosts/${hostId}/heartbeat`, { method: "POST", headers: auth, body: "{}" })).status).toBe(401);
    await prisma.employeeHost.update({ where: { hostId }, data: { expiresAt: new Date(Date.now() + 60_000) } });
    ...
    const good = { operationId, hostId, ...terminal, lease: { computerId: ids.computer, runId: ids.run, fence: 7 }, result: { stdout: "ok", stderr: "", code: 0 } };
